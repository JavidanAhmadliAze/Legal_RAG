from src.sources.isap import ActMeta, filter_foreign, filter_labor
...
def test_filter_labor_matches_kodeks_pracy():
    acts = [_make("Ustawa z dnia 26 czerwca 1974 r. – Kodeks pracy")]
    assert len(filter_labor(acts)) == 1


def test_filter_labor_matches_pracownicy():
    acts = [_make("Rozporządzenie w sprawie wynagrodzenia pracowników")]
    assert len(filter_labor(acts)) == 1


def test_filter_labor_matches_zatrudnienie():
    acts = [_make("Ustawa o promocji zatrudnienia i instytucjach rynku pracy")]
    assert len(filter_labor(acts)) == 1


def test_filter_labor_excludes_unrelated():
    acts = [_make("Ustawa o podatku od towarów i usług")]
    assert len(filter_labor(acts)) == 0


def test_filter_labor_case_insensitive():
    acts = [_make("USTAWA O RYNKU PRACY I SŁUŻBACH ZATRUDNIENIA")]
    assert len(filter_labor(acts)) == 1
